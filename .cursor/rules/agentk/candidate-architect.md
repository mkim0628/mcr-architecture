...
- **`design-point/design-points.md`를 우선 입력으로 사용** — 이미 도출된 설계 포인트(DP-nnn)가 해결해야 할 구조적 문제/설계 이슈의 출발점
- 각 설계 포인트(DP-nnn)를 해당 관심사 측면에서 해결해야 할 설계 이슈로 매핑
- design-points.md에 없는 누락된 구조적 문제가 있는지 추가 분석 (완전성 확보)
- 모든 문제에 대한 해결책 검토

**산출물**:

- 해결해야 할 구조적 문제, 설계 이슈 목록 (각 항목이 어떤 DP-nnn에서 비롯되었는지 추적)

**검증 기준**:

- [ ] 관련 설계 포인트(DP-nnn)가 모두 설계 이슈로 다루어졌는가?
- [ ] 누락된 구조적 문제가 없는가?
- [ ] 각 문제가 명확하게 정의되었는가?
...
- **design-point/design-points.md**: 설계 포인트 — 해결해야 할 구조적 문제 목록 (입력)
