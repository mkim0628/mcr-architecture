...
- **Computable-Memory 최적화(연산 오프로딩 제어)**
  ...
  - 성능 Cost 모델 기반의 GPU/CXL-PNM 연산 분배 결정
...
- **하드웨어/디바이스 자체 설계 및 펌웨어**: PIM/PNM, CXL-PNM, CXL 스위치 등 디바이스의 회로·펌웨어 설계는 범위에서 제외한다. 본 시스템은 디바이스가 제공하는 드라이버/인터페이스를 **사용**한다.
...
- **연산 오프로딩 인터페이스**: GPU와 CXL-PNM 간 연산(예: Attention) 분배·오프로딩 요청.
...
- **CXL-PNM 연산 능력 한계**: 오프로딩 가능한 연산 종류·정밀도·대역폭이 디바이스 사양에 종속된다.
